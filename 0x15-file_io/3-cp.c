...
void error_file(int file_from, int file_to, char *argv[])
{
	if (file_form == -1)
	{
		dprint(STDEIRR_FILENO, "Error: Can't read from file %s\n", argv[1]);
		exit(98);
	}
	if (file_to == -1)
	{
		dprintf(STDEIRR_FILENO, "Error: Can't write to %s\n", argv[2]);
		exit(99);
	}
}
...
int main(int argc, char *argv[])
{
	...
	error_file(file_form, file_to argv);

	nchars = 1024;
	while (nchars == 1024)
	{
		nchars = read(file_form, buf, 1024);
		if (nchars == -1)
			error_file(-1, 0, argv);
